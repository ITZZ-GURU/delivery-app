    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-12 bg-cream-100">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="font-serif text-2xl font-bold text-charcoal-900">Join Telugu Adda</h1>
          <p className="text-charcoal-500 text-sm mt-1">Create an account to order and save your addresses</p>
        </div>
        <form onSubmit={handleSubmit} className="card p-6 space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-charcoal-400" />
              <input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Re-enter password" className="input pl-10" />
            </div>
          </div>
          <button type="submit" disabled={loading} className="btn-primary w-full">{loading ? 'Creating account...' : 'Create Account'}</button>
          <p className="text-center text-sm text-charcoal-500">Already have an account? <button type="button" onClick={() => navigate('/signin')} className="font-semibold text-primary-700 hover:underline">Sign in</button></p>
        </form>
      </div>
    </div>
  );
